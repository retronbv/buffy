...
server.on('request', async (req, res) => {
    ...
    res.writeHead(asset.status, { "Content-Type": contentType.parse(asset.headers.get("content-type")).type }); // Send the HTTP status code set the MIME type
...
server.on('listening', () => {
  ...
  if (process.env.REPL_SLUG && process.env.REPL_OWNER) console.log("Link to view: " + kuler(`https://${process.env.REPL_SLUG}.${process.env.REPL_OWNER}.repl.co`, "#0000ff"));
  else {
    console.log(kuler(`Links to view: http://${hostname()}:${PORT}`, "#0000ff"));
